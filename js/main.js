document.addEventListener('DOMContentLoaded', function () {
  ...
  // --- Favorites ---
  var favorites = JSON.parse(localStorage.getItem('smile_favs') || '[]');
  var favSidebar = document.getElementById('favSidebar');
  var favOverlayEl = document.getElementById('favOverlay');
  ...
  function saveFavs() {
    localStorage.setItem('smile_favs', JSON.stringify(favorites));
  }

  function openFavs() {
    favSidebar.classList.add('active');
    favOverlayEl.classList.add('active');
    document.body.style.overflow = 'hidden';
  }

  function closeFavs() {
    favSidebar.classList.remove('active');
    favOverlayEl.classList.remove('active');
    document.body.style.overflow = '';
  }

  document.getElementById('favToggle').addEventListener('click', openFavs);
  document.getElementById('favClose').addEventListener('click', closeFavs);
  favOverlayEl.addEventListener('click', closeFavs);

  function updateFavUI() {
    favCountEl.textContent = favorites.length;

    var oldItems = favItemsEl.querySelectorAll('.cart-item');
    ...
    favorites.forEach(function (fav, index) {
      ...
      div.innerHTML =
        '<img src="' + fav.img + '" alt="' + fav.name + '" class="cart-item-img">' +
        '<div class="cart-item-details">' +
          '<p class="cart-item-name">' + fav.name + '</p>' +
          '<p class="cart-item-price">' + fav.priceStr + '</p>' +
          '<button class="fav-item-buy" data-index="' + index + '">Comprar</button>' +
        '</div>' +
        '<button class="cart-item-remove" data-index="' + index + '">&times;</button>';
      favItemsEl.appendChild(div);
    });

    favItemsEl.querySelectorAll('.fav-item-buy').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var i = parseInt(this.getAttribute('data-index'));
        var fav = favorites[i];
        closeFavs();
        openProductModal(fav.name, fav.price, fav.img);
      });
    });

    favItemsEl.querySelectorAll('.cart-item-remove').forEach(function (btn) {
      btn.addEventListener('click', function () {
        var i = parseInt(this.getAttribute('data-index'));
        var removed = favorites.splice(i, 1)[0];
        saveFavs();
        updateFavUI();
        document.querySelectorAll('.fav-btn').forEach(function (fb) {
          var card = fb.closest('.product-card');
          if (card && card.querySelector('.product-name').textContent === removed.name) {
  ...
  document.querySelectorAll('.fav-btn').forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      ...
      var price = parseFloat(card.querySelector('.add-to-cart-btn').getAttribute('data-price'));
      var priceStr = card.querySelector('.product-price').textContent;
      var img = card.querySelector('.product-image img').getAttribute('src');

      var existingIndex = favorites.findIndex(function (f) { return f.name === name; });
      ...
      if (existingIndex !== -1) {
        ...
        favorites.push({ name: name, price: price, priceStr: priceStr, img: img });
        this.classList.remove('active');
  ...
  favorites.forEach(function (fav) {
    document.querySelectorAll('.product-card').forEach(function (card) {
      if (card.querySelector('.product-name').textContent === fav.name) {
